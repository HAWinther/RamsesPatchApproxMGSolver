...
subroutine make_grid_fine(ind_grid,ind_cell,ind,ilevel,nn,ibound,boundary_region)
  ...
  use hydro_commons
#if defined(MGSOLVER)
  use poisson_commons, ONLY:f,phi,phi_old,f_mg,phi_mg,phi_mg_old,mg_solver_active
#else
  ...
  if(.not.init .and. .not.balance)then
     ...
     if(mg_solver_active)then
        ...
        do j=1,twotondim
           ...
           do idim=1,ndim
              do i=1,nn
                 f_mg(iskip+ind_grid_son(i),idim)=f_mg(ind_fathers(i,0),idim)
              end do
           end do
           do i=1,nn
              phi_mg(iskip+ind_grid_son(i))=phi_mg(ind_fathers(i,0))
              phi_mg_old(iskip+ind_grid_son(i))=phi_mg_old(ind_fathers(i,0))
           end do
        end do
     end if
#endif
...
subroutine kill_grid(ind_cell,ilevel,nn,ibound,boundary_region)
  ...
  do ind=1,twotondim
     ...
     if(mg_solver_active)then
        do i=1,nn
           phi_mg(ind_cell_son(i))=0.0D0
           phi_mg_old(ind_cell_son(i))=0.0D0
        end do
        do idim=1,ndim
           do i=1,nn
              f_mg(ind_cell_son(i),idim)=0.0D0
           end do
        end do
     end if
#endif
