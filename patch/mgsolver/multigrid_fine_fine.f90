...
#if defined(MGSOLVER)
   real(dp) :: scalar_mass_term, calc_scalar_mass_term

   if(mg_solver_active .and. use_poisson_solver_for_mg) then
     scalar_mass_term = calc_scalar_mass_term(aexp)
   else
     scalar_mass_term = 0.0d0
   endif
#endif
...
#if defined(MGSOLVER)
         ! If we use the Poisson solver for another linear equation
         f(icell_amr,1) = f(icell_amr,1) + scalar_mass_term * phi_c
#endif
   ...
   do ind0=1,twotondim/2      ! Only half of the cells for a red or black sweep
      ...
      do igrid_mg=1,ngrid
         ...
         if(flag2(icell_amr)/ngridmax==0) then
            ...
            phi(icell_amr) = (nb_sum - dx2*f(icell_amr,2)) / (dtwondim + dx2 * scalar_mass_term)
#else
            ! Update the potential, solving for potential on icell_amr
...
#if defined(MGSOLVER)
            ! Update the potential, solving for potential on icell_amr
            phi(icell_amr) = (nb_sum - dx2*f(icell_amr,2)) / (dtwondim - weight + dx2 * scalar_mass_term)
#else
            ! Update the potential, solving for potential on icell_amr
