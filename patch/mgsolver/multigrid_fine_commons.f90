...
subroutine multigrid_fine(ilevel,icount)
   ...
   main_iteration_loop: do
      ...
      ! Converged?
#if defined(MGSOLVER)
      if(mg_solver_active .and. use_poisson_solver_for_mg) then
        if(err<epsilon_mg .or. iter>=MAXITER) exit
      else
...
#if defined(MGSOLVER)
         if(mg_solver_active .and. use_poisson_solver_for_mg) then
            ! Rescale Poisson equation source by scalar_coupling = 2beta(a)^2
            f(icell_amr,2) = f(icell_amr,2) * scalar_coupling

            ! Use the screening method of Winther & Ferreira 2014 and change
            ! density to effective density = density * screening_function(Phi_newton)
            if(include_screening) then
              f(icell_amr,2) = f(icell_amr,2) * &
                calc_screening(phi_array(icell_amr), rho(icell_amr), aexp)
            endif
         endif
#endif
