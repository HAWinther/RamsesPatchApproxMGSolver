  ...
  do i1=i1min,i1max
     ...
     do i2=i2min,i2max
        ...
        if(mg_solver_active)then
           do idim=1,ndim
              do i=1,nexist
                 gloc(ind_exist(i),i3,j3,k3,idim)=gloc(ind_exist(i),i3,j3,k3,idim) &
                  + f_mg(ind_cell(i),idim)
              end do
              ! Use straight injection for buffer cells
              do i=1,nbuffer
                 gloc(ind_nexist(i),i3,j3,k3,idim)=gloc(ind_nexist(i),i3,j3,k3,idim) &
                   + f_mg(ibuffer_father(i,0),idim)
              end do
           end do
        end if
#endif
