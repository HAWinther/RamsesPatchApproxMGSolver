...
#if defined(MGSOLVER)
  real(dp) :: scalar_coupling, calc_scalar_coupling
  real(dp) :: calc_screening, calc_scalar_mass_term, max_mod_fac
#endif
...
#if defined(MGSOLVER)
              if(mg_solver_active)then

                 ! Switch pointers
                 phi=>phi_mg_array; phi_old=>phi_mg_old_array; f=>f_mg_array
                 ...
                 ! Switch pointers back
                 phi=>phi_array; phi_old=>phi_old_array; f=>f_array
              end if
#endif
           end if
...
#if defined(MGSOLVER)
    if(mg_solver_active) then
       call timer('modifiedgravity','start')

       ! Switch pointers
       phi => phi_mg_array; phi_old => phi_mg_old_array; f => f_mg_array
       
       ! The maximum Geff(k,a)/G - 1 on the domaingrid (k = kmax = 2pi/Box * Ngrid)
       scalar_coupling = calc_scalar_coupling(aexp)
       max_mod_fac = abs(scalar_coupling) / (1.0 + calc_scalar_mass_term(aexp) / (2.0 * acos(-1.0) * 2**ilevel)**2 )
       
       if(myid==1 .and. ilevel==levelmin) then
         write(*,*) ''
         write(*,*) '=============================================================='
         write(*,*) 'Solve for modified gravity potential        '
         write(*,*) '=============================================================='
         write(*,*) 'Scalefactor                    = ', aexp
         write(*,*) 'Scalar coupling 2beta^2(a)     = ', scalar_coupling
         write(*,*) 'Scalar mass [am]^{-1} (Mpc/h)  = ', boxlen_ini/sqrt(calc_scalar_mass_term(aexp))
         write(*,*) 'Max [Geff(k,a)/G - 1]          = ', max_mod_fac 
       endif

       ! We do not need to solve when e.g. max_mod_fac < 0.001 so we can skip this
       if(max_mod_fac > eps_to_start_solving .and. ilevel == levelmin) then
         mg_solver_has_started = .true.
         if(myid == 1) then
           write(*,*) '=============================================================='
           write(*,*) ''
         endif
         ...
         if(myid == 1) then
           write(*,*) 'Max value <', eps_to_start_solving,' so we skip solving!'
           write(*,*) '=============================================================='
           write(*,*) ''
         endif
       endif

       if(mg_solver_has_started) then
         
         ! Activate modifications
         use_poisson_solver_for_mg = .true.

         ! Compute fifth-force potential
         if(ilevel>levelmin)then
           if(ilevel .ge. cg_levelmin) then
             call phi_fine_cg(ilevel,icount)
           else
         ...
         ! Compute fifth-force acceleration
         call force_fine(ilevel,icount)

         ! Deactivate modifications
         use_poisson_solver_for_mg = .false.
